...
namespace {
inline void hash_update_GF2E(hash_context *ctx, const field::GF2E32 &element) {
  std::array<uint8_t, field::GF2E32::BYTE_SIZE> buffer;
  element.to_bytes(buffer.data());
  hash_update(ctx, buffer.data(), field::GF2E32::BYTE_SIZE);
}
...
phase_1_commitment(const banquet_instance_t &instance,
                   ...
                   const std::vector<std::vector<field::GF2E32>> &t_deltas,
  ...
  for (size_t repetition = 0; repetition < instance.num_rounds; repetition++) {
    ...
    for (auto &t_delta : t_deltas[repetition]) {
      hash_update_GF2E(&ctx, t_delta);
    }
  }
  hash_final(&ctx);
...
std::vector<std::vector<field::GF2E32>>
...
phase_2_commitment(const banquet_instance_t &instance,
                   ...
                   const std::vector<std::vector<field::GF2E32>> &P_deltas) {
...
phase_2_expand(const banquet_instance_t &instance,
  ...
  std::array<uint8_t, field::GF2E32::BYTE_SIZE> buffer;
  std::vector<field::GF2E32> R_es;
  for (size_t e = 0; e < instance.num_rounds; e++) {
    while (true) {
      hash_squeeze(&ctx, buffer.data(), buffer.size());
      //  check that R is not in {0,...m2-1}
      field::GF2E32 candidate_R;
...
phase_3_commitment(const banquet_instance_t &instance,
                   ...
                   const std::vector<field::GF2E32> &c,
                   const std::vector<std::vector<field::GF2E32>> &c_shares,
                   const std::vector<std::vector<field::GF2E32>> &a,
                   const RepContainer<field::GF2E32> &a_shares,
                   const std::vector<std::vector<field::GF2E32>> &b,
                   const RepContainer<field::GF2E32> &b_shares) {
  ...
  for (size_t repetition = 0; repetition < instance.num_rounds; repetition++) {
    ...
    for (size_t j = 0; j < instance.m1; j++) {
      ...
      for (size_t party = 0; party < instance.num_MPC_parties; party++) {
        hash_update_GF2E(&ctx, a_shares.get(repetition, party)[j]);
        hash_update_GF2E(&ctx, b_shares.get(repetition, party)[j]);
      }
    }
  }
  hash_final(&ctx);
...
banquet_signature_t banquet_sign(const banquet_instance_t &instance,
  ...
  std::pair<std::vector<field::GF2E32>, std::vector<field::GF2E32>> sbox_pairs;
  ...
  size_t random_tape_size = instance.aes_params.key_size +
                            instance.aes_params.num_sboxes +
                            2 * instance.m1 * field::GF2E32::BYTE_SIZE +
                            (instance.m2 + 1) * field::GF2E32::BYTE_SIZE;
  ...
  /////////////////////////////////////////////////////////////////////////////
  // phase 1: commit to executions of LS-AES
  /////////////////////////////////////////////////////////////////////////////
  ...
  RepByteContainer rep_output_broadcasts(
      ...
      instance.aes_params.block_size * instance.aes_params.num_blocks);
  RepContainer<field::GF2E32> rep_shared_s(instance.num_rounds,
                                           instance.num_MPC_parties,
                                           instance.aes_params.num_sboxes);
  RepContainer<field::GF2E32> rep_shared_t(instance.num_rounds,
                                           instance.num_MPC_parties,
                                           instance.aes_params.num_sboxes);
  std::vector<std::vector<uint8_t>> rep_key_deltas;
  std::vector<std::vector<field::GF2E32>> rep_t_deltas;
  ...
  for (size_t repetition = 0; repetition < instance.num_rounds; repetition++) {
    ...
    std::vector<field::GF2E32> t_deltas = sbox_pairs.second;
    for (size_t party = 0; party < instance.num_MPC_parties; party++) {
  ...
  std::vector<std::vector<field::GF2E32>> r_ejs = phase_1_expand(instance, h_1);
  ...
  std::vector<field::GF2E32> x_values_for_interpolation_zero_to_m2 =
      field::get_first_n_field_elements(instance.m2 + 1);
  std::vector<std::vector<field::GF2E32>> precomputation_for_zero_to_m2 =
      field::precompute_lagrange_polynomials(
          x_values_for_interpolation_zero_to_m2);
  std::vector<field::GF2E32> x_values_for_interpolation_zero_to_2m2 =
      field::get_first_n_field_elements(2 * instance.m2 + 1);
  std::vector<std::vector<field::GF2E32>> precomputation_for_zero_to_2m2 =
      field::precompute_lagrange_polynomials(
          x_values_for_interpolation_zero_to_2m2);

  std::vector<std::vector<std::vector<std::vector<field::GF2E32>>>> s_prime(
      instance.num_rounds);
  std::vector<std::vector<std::vector<std::vector<field::GF2E32>>>> t_prime(
      instance.num_rounds);
  ...
  std::vector<std::vector<field::GF2E32>> P_e(instance.num_rounds);
  std::vector<std::vector<std::vector<field::GF2E32>>> P_e_shares(
      instance.num_rounds);
  // std::vector<std::vector<GF2EX>> P_ei(instance.num_rounds);
  std::vector<std::vector<field::GF2E32>> P_deltas(instance.num_rounds);

  // polynomials for adjusting last S evaluation
  std::vector<field::GF2E32> last_lagrange =
      precomputation_for_zero_to_m2[instance.m2];
  std::vector<field::GF2E32> last_lagrange_sq = last_lagrange * last_lagrange;

  // vectors of intermediate product polynomials for computing P
  std::vector<std::vector<field::GF2E32>> ST_products(instance.m1);
  std::vector<std::vector<field::GF2E32>> S_lag_products(instance.m1);
  std::vector<std::vector<field::GF2E32>> T_lag_products(instance.m1);

  std::vector<std::vector<field::GF2E32>> s_random_points(instance.num_rounds),
      t_random_points(instance.num_rounds);

  // rearrange s-box values into polynomials
  for (size_t j = 0; j < instance.m1; j++) {
    std::vector<field::GF2E32> S_poly(instance.m2 + 1);
    std::vector<field::GF2E32> T_poly(instance.m2 + 1);
    for (size_t k = 0; k < instance.m2; k++) {
      S_poly[k] = sbox_pairs.first[j + instance.m1 * k];
      T_poly[k] = sbox_pairs.second[j + instance.m1 * k];
    }
  ...
  for (size_t repetition = 0; repetition < instance.num_rounds; repetition++) {
    ...
    for (size_t party = 0; party < instance.num_MPC_parties; party++) {
      ...
      // rearrange shares
      std::vector<field::GF2E32> s_bar;
      std::vector<field::GF2E32> t_bar;
      s_bar.resize(instance.m2 + 1);
      t_bar.resize(instance.m2 + 1);

      for (size_t j = 0; j < instance.m1; j++) {
        for (size_t k = 0; k < instance.m2; k++) {
          s_bar[k] = r_ejs[repetition][j] * shared_s[j + instance.m1 * k];
          t_bar[k] = shared_t[j + instance.m1 * k];
        }
        ...
        auto S_T_bar = random_tapes.get_bytes(
            ...
            instance.aes_params.key_size + instance.aes_params.num_sboxes +
                j * 2 * field::GF2E32::BYTE_SIZE,
            2 * field::GF2E32::BYTE_SIZE);

        s_bar[instance.m2].from_bytes(S_T_bar.data());
        t_bar[instance.m2].from_bytes(S_T_bar.data() +
                                      field::GF2E32::BYTE_SIZE);
    ...
    std::vector<field::GF2E32> P(2 * instance.m2 + 1);
    ...
    std::vector<std::vector<field::GF2E32>> &P_shares = P_e_shares[repetition];
    ...
    for (size_t party = 0; party < instance.num_MPC_parties; party++) {
      ...
      if (party == 0) {
        ...
        for (size_t k = 0; k < instance.m2; k++) {
          P_shares[party][k] = field::GF2E32(0);
        }
      }
      ...
      auto random_P_shares = random_tapes.get_bytes(
          repetition, party,
          instance.aes_params.key_size + instance.aes_params.num_sboxes +
              instance.m1 * 2 * field::GF2E32::BYTE_SIZE,
          (instance.m2 + 1) * field::GF2E32::BYTE_SIZE);
    ...
    for (size_t k = instance.m2; k <= 2 * instance.m2; k++) {
      // calculate offset
      field::GF2E32 k_element = x_values_for_interpolation_zero_to_2m2[k];
      field::GF2E32 P_at_k_delta = field::eval(P, k_element);
      for (size_t party = 0; party < instance.num_MPC_parties; party++) {
  ...
  std::vector<field::GF2E32> forbidden_challenge_values =
  ...
  std::vector<field::GF2E32> c(instance.num_rounds);
  std::vector<std::vector<field::GF2E32>> c_shares(instance.num_rounds);
  std::vector<std::vector<field::GF2E32>> a(instance.num_rounds);
  std::vector<std::vector<field::GF2E32>> b(instance.num_rounds);
  RepContainer<field::GF2E32> a_shares(instance.num_rounds,
                                       instance.num_MPC_parties, instance.m1);
  RepContainer<field::GF2E32> b_shares(instance.num_rounds,
                                       instance.num_MPC_parties, instance.m1);

  std::vector<field::GF2E32> lagrange_polys_evaluated_at_Re_m2(instance.m2 + 1);
  std::vector<field::GF2E32> lagrange_polys_evaluated_at_Re_2m2(
      2 * instance.m2 + 1);
...
bool banquet_verify(const banquet_instance_t &instance,
  ...
  // recompute h_2
  std::vector<std::vector<field::GF2E32>> P_deltas;
  ...
  for (size_t repetition = 0; repetition < instance.num_rounds; repetition++) {
    ...
    for (size_t party = 0; party < instance.num_MPC_parties; party++) {
      if (party != missing_parties[repetition]) {
        ...
        // rearrange shares
        std::vector<field::GF2E32> s_bar(instance.m2 + 1);
        std::vector<field::GF2E32> t_bar(instance.m2 + 1);
  ...
  std::vector<std::vector<field::GF2E32>> t_deltas;
  for (const banquet_repetition_proof_t &proof : signature.proofs) {
...
banquet_serialize_signature(const banquet_instance_t &instance,
  ...
  for (size_t repetition = 0; repetition < instance.num_rounds; repetition++) {
    ...
    serialized.insert(serialized.end(), proof.sk_delta.begin(),
                      proof.sk_delta.end());
    for (size_t k = 0; k < instance.aes_params.num_sboxes; k++) {
      std::array<uint8_t, field::GF2E32::BYTE_SIZE> buffer =
          proof.t_delta[k].to_bytes();
      serialized.insert(serialized.end(), buffer.begin(), buffer.end());
    }
    for (size_t k = 0; k < instance.m2 + 1; k++) {
      std::array<uint8_t, field::GF2E32::BYTE_SIZE> buffer =
          proof.P_delta[k].to_bytes();
      serialized.insert(serialized.end(), buffer.begin(), buffer.end());
    }
    {
      std::array<uint8_t, field::GF2E32::BYTE_SIZE> buffer =
          proof.P_at_R.to_bytes();
      serialized.insert(serialized.end(), buffer.begin(), buffer.end());
    }
    for (size_t j = 0; j < instance.m1; j++) {
      std::array<uint8_t, field::GF2E32::BYTE_SIZE> buffer =
          proof.S_j_at_R[j].to_bytes();
      serialized.insert(serialized.end(), buffer.begin(), buffer.end());
    }
    for (size_t j = 0; j < instance.m1; j++) {
      std::array<uint8_t, field::GF2E32::BYTE_SIZE> buffer =
          proof.T_j_at_R[j].to_bytes();
      serialized.insert(serialized.end(), buffer.begin(), buffer.end());
    }
  }
  return serialized;
...
banquet_deserialize_signature(const banquet_instance_t &instance,
  ...
  for (size_t repetition = 0; repetition < instance.num_rounds; repetition++) {
    ...
    field::GF2E32 tmp;
    std::vector<field::GF2E32> t_delta;
    t_delta.reserve(instance.aes_params.num_sboxes);
    for (size_t k = 0; k < instance.aes_params.num_sboxes; k++) {
      std::array<uint8_t, field::GF2E32::BYTE_SIZE> buffer;
    ...
    std::vector<field::GF2E32> P_delta;
    ...
    std::vector<field::GF2E32> S_j_at_R;
    ...
    std::vector<field::GF2E32> T_j_at_R;
